...
const Employees = () => {
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        if (uploadsToPerform.length > 0) {
          ...
          const uploadPromises = uploadsToPerform.map(async ({ documentType, file }) => {
  ...
  const renderExistingDocumentsWarning = (documentType) => {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ fontWeight: '600', marginBottom: '8px', color: '#0056b3', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span>📋</span>
        ...
        <div style={{ marginTop: '8px', fontSize: '12px', color: '#0056b3', fontStyle: 'italic' }}>
  ...
  const renderNewFileIndicator = (file) => {
    if (!file) return null;
    return (
      <div style={{ marginTop: '6px', padding: '6px 8px', background: '#d4edda', border: '1px solid #28a745', borderRadius: '4px', fontSize: '11px' }}>
        <strong>✓ سيتم رفع:</strong> {file.name || (Array.isArray(file) ? `${file.length} ملف` : 'مستند')}
      </div>
    );
  };
    ...
    if (filesToCheck.length > 0) {
      for (const file of filesToCheck) {
        ...
        if (file.size > maxSize) {
          showWarning(`حجم الملف "${file.name}" كبير جداً. الحد الأقصى لحجم الملف هو 1 ميجابايت.`);
    ...
    setFormData({
      ...
      id_expiry_date_gregorian: (() => {
        ...
        if (employee.id_expiry_date_hijri) {
          const [day, month, year] = employee.id_expiry_date_hijri.split('/');
          return hijriToGregorian(day, month, year) || '';
        }
        return '';
      })(),
              ...
              <tbody>
                {employees.length === 0 ? (
                  ...
                  paginatedEmployees.map((employee) => {
                    const branch = branchesMap.get(employee.branch_id);
                    const isComplete = employee.data_completion_status === DATA_COMPLETION_STATUS.COMPLETE;
                    const completionStatus = employee.data_completion_status || DATA_COMPLETION_STATUS.INCOMPLETE;
                    return (
                      <tr key={employee.id}>
                    ...
                    <div className="documents-section col-12">
                      ...
                      <div className="form-group col-3">
                        ...
                        <input
                          ...
                          onChange={(e) => handleDocumentChange('id_or_residency', e)}
                        />
                        {documents.id_or_residency && (
                          <div style={{ marginTop: '6px', padding: '6px 8px', background: '#d4edda', border: '1px solid #28a745', borderRadius: '4px', fontSize: '11px' }}>
                            <strong>✓ سيتم رفع:</strong> {documents.id_or_residency.name}
                          </div>
                        )}
                      </div>
                      <div className="form-group col-3">
                        ...
                        <input
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png"
                          onChange={(e) => handleDocumentChange('direct_letter', e)}
                        />
                      ...
                      <div className="form-group col-3">
                        ...
                        <input
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png"
                          onChange={(e) => handleDocumentChange('employment_contract', e)}
                        />
                      ...
                      <div className="form-group col-3">
                        ...
                        {renderExistingDocumentsWarning('medical_disclosure_form')}
                        <input
                          type="file"
                          accept=".pdf,.jpg,.jpeg,.png"
                          onChange={(e) => handleDocumentChange('medical_disclosure_form', e)}
                        />
                      ...
                      {isNonSaudi(formData.nationality) && (
                        <div className="form-group col-3">
                          ...
                          <input
                            type="file"
                            accept=".pdf,.jpg,.jpeg,.png"
                            onChange={(e) => handleDocumentChange('passport', e)}
                          />
                      ...
                      {isSchool(currentBranchType) && (
                        <>
                          <div className="form-group col-3">
                            ...
                            {renderExistingDocumentsWarning('professional_license')}
                            <input
                              type="file"
                              accept=".pdf,.jpg,.jpeg,.png"
                              onChange={(e) => handleDocumentChange('professional_license', e)}
                            />
                            {renderNewFileIndicator(documents.professional_license)}
                          </div>
                            ...
                            <div className="form-group col-3">
                              ...
                              {existingDocuments.experience_certificate && existingDocuments.experience_certificate.length > 0 && (
                                <div style={{ 
                                  ...
                                  <div style={{ marginTop: '8px', fontSize: '12px', color: '#0056b3', fontStyle: 'italic' }}>
                                    ℹ️ إضافة ملفات جديدة لن تؤثر على الملفات الموجودة (يمكن إضافة حتى 5 ملفات)
                                  </div>
                                </div>
                              )}
                              ...
                              {(!documents.experience_certificate || documents.experience_certificate.length < 5) && (
                                <input
                                  type="file"
                                  accept=".pdf,.jpg,.jpeg,.png"
                                  onChange={(e) => {
                                    if (e.target.files[0]) {
                                      handleDocumentChange('experience_certificate', e.target.files[0]);
                                      e.target.value = '';
                                    }
                                  }}
                                />
                        ...
                        <>
                          ...
                          {requiresClassificationDocument(formData.job_title) && (
                            <div className="form-group col-3">
                              ...
                              <input
                                type="file"
                                accept=".pdf,.jpg,.jpeg,.png"
                                onChange={(e) => handleDocumentChange('classification', e)}
                              />
                          ...
                          {formData.job_title === 'النطق و التخاطب' && (
                            <>
                              <div className="form-group col-3">
                                ...
                                {renderExistingDocumentsWarning('speech_therapy_course')}
                                <input
                                  type="file"
                                  accept=".pdf,.jpg,.jpeg,.png"
                                  onChange={(e) => handleDocumentChange('speech_therapy_course', e)}
                                />
                                {renderNewFileIndicator(documents.speech_therapy_course)}
                              </div>
                              {/* 70 hours speech therapy course - required for speech therapists */}
                              <div className="form-group col-3">
                                <label>دورة 70 ساعة في التخاطب</label>
                                {renderExistingDocumentsWarning('speech_therapy_70_hours_course')}
                                <input
                                  type="file"
                                  accept=".pdf,.jpg,.jpeg,.png"
                                  onChange={(e) => handleDocumentChange('speech_therapy_70_hours_course', e)}
                                />
                                {renderNewFileIndicator(documents.speech_therapy_70_hours_course)}
                              </div>
                            </>
                          )}
                          ...
                          {(formData.job_title === 'علاج طبيعي' || formData.job_title === 'علاج وظيفي') && (
                            <div className="form-group col-3">
                              ...
                              {renderExistingDocumentsWarning('physical_therapy_course')}
                              <input
                                type="file"
                                accept=".pdf,.jpg,.jpeg,.png"
                                onChange={(e) => handleDocumentChange('physical_therapy_course', e)}
                              />
                              {renderNewFileIndicator(documents.physical_therapy_course)}
                            </div>
                          )}
                          ...
                          {requiresTherapy40HoursDocument(formData.job_title) && (
                            <div className="form-group col-3">
                              ...
                              {renderExistingDocumentsWarning('therapy_40_hours_course')}
                              <input
                                type="file"
                                accept=".pdf,.jpg,.jpeg,.png"
                                onChange={(e) => handleDocumentChange('therapy_40_hours_course', e)}
                              />
                              {renderNewFileIndicator(documents.therapy_40_hours_course)}
                            </div>
                          )}
                          <div className="form-group col-3">
                            <label>الدورات الإضافية (يمكن إضافة حتى 5 ملفات)</label>
